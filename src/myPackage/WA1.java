...
public class WA1 {
  ...
  public static int solution(final int[][] passwords, final String s) {
    final Map<Integer, Integer> map = new HashMap<>(passwords.length);
    for (final int[] password : passwords) {
    ...
    while (tokenizer.hasMoreTokens()) {
      final int houseNumber = Integer.parseInt(tokenizer.nextToken());
      if (!tokenizer.hasMoreTokens()) {
        return answer;
      }

      if (map.containsKey(houseNumber)) {
        final Integer password = map.get(houseNumber);
        final int input = Integer.parseInt(tokenizer.nextToken());
        if (input == password) {
          answer++;
        }
      }
